...
RUNTIME_RESOURCE_URLS=$(curl -s -X GET --header "Accept:application/json" --header "Content-Type:application/json;" $API_ENDPOINT_URL/microservices/runtimes | jq -r ._embedded.runtimes[]._links.self.href)
...
ARCHITECTURE_RESOURCE_URLS=$(curl -s -X GET --header "Accept:application/json" --header "Content-Type:application/json;" $API_ENDPOINT_URL/microservices/architectures | jq -r ._embedded.architectures[]._links.self.href)
...
FEATURE_RESOURCE_URLS=$(curl -s -X GET --header "Accept:application/json" --header "Content-Type:application/json;" $API_ENDPOINT_URL/microservices/features | jq -r ._embedded.features[]._links.self.href)
